...
| `sitemapLastmod`, `sitemapLastmodRecent` | ✅ `wp-sitemap.xml` emits `<lastmod>` for every post-type entry and the posts-index homepage since **6.5** (`class-wp-sitemaps-posts.php`, from `post_modified_gmt`); term and author entries still carry none | ✅ XML Sitemaps emit per-URL `<lastmod>` — **free** (Jetpack → SEO → Sitemaps), refreshed ~12h. Confirmed in Jetpack's `sitemap-builder.php` (`jp_sitemap_datetime()`). | Core covers it on WP ≥ 6.5. Lastmod absent entirely means a pre-6.5 core or a plugin sitemap that replaced it — update WordPress or enable Jetpack Sitemaps. Lastmod present but stale means stale content; the fix is updating the content, not the sitemap. |
...
- WordPress core XML sitemaps — <https://make.wordpress.org/core/2020/07/22/new-xml-sitemaps-functionality-in-wordpress-5-5/>; lastmod for post-type entries and the homepage landed in 6.5 (Trac <https://core.trac.wordpress.org/ticket/52099>). Verify by diffing `src/wp-includes/sitemaps/providers/class-wp-sitemaps-posts.php` at the `6.4` and `6.5` tags of `WordPress/wordpress-develop`.
...
**Sources of record (last verified 2026-07-15):** Jetpack SEO Tools, Jetpack Sitemaps support doc + `sitemap-builder.php` source (per-URL lastmod), Jetpack Open Graph developer hook, WordPress 5.5 core sitemaps announcement + Trac #52099 (core emits lastmod for post-type entries and the homepage since 6.5; term and author entries omit it), WordPress core JSON-LD-by-default (none).
