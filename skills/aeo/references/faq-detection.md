...
function koshFaqProbe() {
  ...
  const isAnswerLike = el => !!el && ANSWER_TAGS.test(el.tagName) && txt(el).length > MIN_ANSWER_CHARS;
  const hasAnswerText = h => {
    ...
    return !!parent && isAnswerLike(parent.nextElementSibling);
  };
  ...
  const hasRdfaFaq = Array.from(document.querySelectorAll('[typeof]')).some(el =>
    isSchemaOrgRdfa(el) &&
...
### Shared RDFa scope test

`isSchemaOrgRdfa` is shared by this probe and the `rdfa` count in `jsonLdFormat` (Section 1.3) so the two can never disagree about whether a page contains schema.org RDFa. Previously the FAQ check accepted any `[typeof~="schema:FAQPage"]` while `jsonLdFormat` required an explicit `vocab`/`prefix` scope, so one report could read "RDFa-only FAQ markup present" next to "no RDFa present" for the same markup.

```javascript
function isSchemaOrgRdfa(el) {
...
**Never fail a site merely for lacking an FAQ.** `faqRelevance: absent` → `na` on both schema signals, no issue generated. That is the expected outcome for the many sites that legitimately have no FAQ.

| Aggregate state | `faqSchema` | `faqSchemaApplied` |
| --- | --- | --- |
| `faqRelevance: absent` | `na` | `na` |
...
| tier `valid`/`weak` but no visible FAQ | `partial` | `partial` |
