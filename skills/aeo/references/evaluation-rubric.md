...
| faqSchema | Visible FAQ content AND `faqSchemaTier` is `valid` (JSON-LD FAQPage with 2+ `mainEntity` `Question` nodes carrying non-empty `acceptedAnswer` text). Both axes are cross-page — see `faq-detection.md`. | Visible FAQ content and `faqSchemaTier` is `weak` (malformed JSON-LD, thin/empty `mainEntity`, or microdata- or RDFa-only FAQ markup), OR `faqRelevance` is `medium` (3+ answered question-framed headings, no FAQ container/heading/accordion) and `faqSchemaTier` is `none`. **The two branches take different `notes`/remedy strings — see the per-branch table in `faq-detection.md`; do not tell a site to add markup it already has.** | Visible FAQ content but `faqSchemaTier` is `none`. (`na` only when the site has no FAQ content **and** `faqSchemaTier` is `none` — never `fail` merely for lacking an FAQ.) |
...
| faqSchemaApplied | Visible FAQ content AND `faqSchemaTier` is `valid`, on whichever page carries each (cross-page, not homepage-only). | `faqSchemaTier` is `weak` with visible FAQ content, OR `faqSchemaTier` is `valid`/`weak` with no visible FAQ content (or vice versa), OR `faqRelevance` is `medium` and `faqSchemaTier` is `none`. | FAQ warranted but neither visible FAQ content nor at least `weak` schema is in place. (`na` only when the site has no FAQ content **and** `faqSchemaTier` is `none`.) |
...
> All FAQ signals above (`faqSchema`, `faqSchemaApplied`, `faqSectionPresent`, and `faqRelevance` feeding `applicableSchemas.FAQPage`) resolve from the one canonical probe in `faq-detection.md`. Change the probe and all four move together — update that file's status table and these rows in the same commit.
