...
| faqSchema | Visible FAQ content AND `faqSchemaTier` is `valid` (JSON-LD FAQPage with 2+ Q&A pairs). Both relevance and schema tier are cross-page — check whichever page (homepage or a dedicated FAQ page) actually carries them. | Visible FAQ content and `faqSchemaTier` is `weak` (malformed JSON-LD, microdata-only, or RDFa-only FAQ markup), OR `faqRelevance` is `medium` (3+ answered question-framed headings, no FAQ container/heading/accordion) and `faqSchemaTier` is `none`. | Visible FAQ content but `faqSchemaTier` is `none`. (`na` when the site has no FAQ content — never `fail` merely for lacking an FAQ.) |
