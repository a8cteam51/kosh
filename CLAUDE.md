...
`reports/` has one location: `KOSH_REPORTS_DIR` if set, else `reports/` next to `scripts/`. Every script resolves it from its own location, never from the cwd, and `tests/reports-dir.test.js` runs the shell scripts from a foreign cwd to keep it that way (the tests also use the variable to stay out of the real directory). Skills are the exception: they write `reports/…` cwd-relative and call `scripts/run-qa-report.sh` cwd-relative, which is why the documented launch is `cd kosh && claude --plugin-dir .`. The SessionStart hook pre-creates the folders as a nicety; nothing depends on it.
